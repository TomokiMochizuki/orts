...
- **探索の間、離散モードは root 前のまま凍結する。** 飽和などでモードが境界で変わる系を、確定
  ...
  収束する。$y' = 1\ (y < 1)$、$y' = 0\ (y \ge 1)$、$g = y - 1$ は真の到達が $t = 1$ だが、
  RK4 の再ステップは $t \approx 1.2$ に収束する。凍結するのは離散モードだけで、連続的な
  state feedback は毎ステージ評価したままにする
...
- 固定刻みの Verlet / Yoshida では、公開する状態・callback・再検出について同じ契約を満たせるが、
  幾何的な性質は保証できない。root のために刻みを縮めると合成の対称性が崩れる
