...
- **1 つのステップが、同じ event の (向きの合う) 交差を 2 つ以上含んではならない。** 検出は
  ステップの始点の値と試行の終点の値を比べるだけなので、2 つ含むと符号差が消えて検出されず、
  3 つ以上では判定が時刻について単調でなくなる。$g = (t-0.2)(t-0.4)(t-0.8)$ を $[0, 1]$ で見ると、
  最初の試行 $0.5$ が始点と同符号になり、$0.2$ と $0.4$ を捨てて $0.8$ に収束する (実測)。
  呼び手が刻みを制限して 1 ステップに 1 つへ収める。探索側からは検査できない
