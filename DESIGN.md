...
- 境界は符号付きの連続関数 $g(t, y)$ で表す。交差の向き (rising / falling / either) と、境界に
  着いたら止まるか (terminal) 続けるか (non-terminal) を event 自身が申告する
- 交差の判定は projection 前の生の候補で行う。projection は制約面に戻す操作なので、符号変化を
  消してしまうことがある
- **探索の間、離散モードは root 前のまま凍結する。** 飽和などでモードが境界で変わる系を、確定
  済みの始点から短い刻みで再計算すると、RK のステージ間でモードが混ざり、二分法は誤った時刻に
  収束する。$y' = 1\ (y < 1)$、$y' = 0\ (y \ge 1)$、$g = y - 1$ で測ると、境界までの残り
  $r = 1 - y_0$ に対して二分法が返す刻みは $w = 6r/5$ で、到達時刻を $0.2r$ だけ遅く報告する。
...
- **1 つのステップが含んでよい符号変化は、各 event の値について、向きを問わず 1 回まで。**
  ...
  途中で反対向きに横切って戻れば符号変化は 2 回なので同じことが起きる。呼び手が刻みを制限して
  1 ステップに 1 回へ収める。探索側からは検査できない
- 局所化は、確定済みの始点から刻みを縮めて再計算する二分法で行う (dense output は持たない)。
  探索中は projection も callback もモード変更も走らせない。adaptive では縮めた候補についても
...
- 再検出の抑止は event ごとに状態を持つ。境界面に沿って動く制約では root の直後も $g \approx 0$
  ...
  導くことはしない (単位が違う)
- 同時に見つかった root は群として返す。処理の順序は呼び手が与える優先度で決め、同順位は安定な
  ID 順にする。勾配の大きさで決めない
...
非 terminal な root の後は、呼び手が state とモードを更新してから積分を再開する。再開の手続き
