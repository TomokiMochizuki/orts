...
- **姿勢表示**: 機体を原点に固定し、参照フレーム軸と方向ベクトル (太陽・nadir) を添えて向きだけを見る表示。ADCS の検証で地球と軌跡が邪魔になるのが動機。設計は [DESIGN.md](DESIGN.md) の「Viewer 表示アーキテクチャ」を参照。方向ベクトルは軌道表示の機体中心ビューでも使う。地磁気ベクトルは tobari が viewer 側に無いので、サーバーが計算して state に載せる別作業になる
